...
//    }
    ...
    //      .map(a => r.reads(Json.parse(a.getValue.toString)))

    waitForGetAndCas(bucket.couchbaseClient.asyncGetAndTouch(key, exp), bucket, ec, r) map {
      case doc: CASValue[String] => Some(r.reads(Json.parse(doc.toString)))
      case null => None
      case _ if bucket.failWithNonStringDoc => throw new IllegalStateException(s"Document '$key' is not a string ...")
      case _ if !bucket.failWithNonStringDoc => None
    }
